...
class DGCNN(nn.Module):
    def __init__(self, xdim, k_adj, num_out, nclass=3):
        ...
        self.dropout1 = nn.Dropout(p=0.3)  # 添加Dropout层，丢弃率为0.5
        self.fc3 = Linear(32, 8)
        self.dropout2 = nn.Dropout(p=0.3)  # 添加Dropout层，丢弃率为0.5
        self.fc4 = Linear(8, nclass)
        self.A = nn.Parameter(torch.FloatTensor(xdim[1], xdim[1]))
        nn.init.xavier_normal_(self.A)

    def forward(self, x):
        x = self.BN1(x.transpose(1, 2)).transpose(1, 2)  # 因为第三维才为特征维度
        L = normalize_A(self.A)  # A是自己设置的xdim[1] * xdim[1]的可训练参数，即邻接矩阵
        result = self.layer1(x, L)
        result = result.reshape(x.shape[0], -1)
        result = F.relu(self.fc1(result))
        result = self.dropout1(result)  # 在全连接层之后添加Dropout
        result = F.relu(self.fc3(result))
        result = self.dropout2(result)  # 在全连接层之后添加Dropout
